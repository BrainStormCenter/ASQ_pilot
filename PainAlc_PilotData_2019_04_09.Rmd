...
p <- ggplot(data2, aes(Groups, max.temp, colour = Groups))
...
group_by(data3$Groups) %>% 
	summarise(MAXXtemp = count(data3$max.temp)
			  )
...
data3 %>%
	group_by(data3$Groups) %>%
		summarise(
			avg_temp = mean(data3$max.temp, na.rm = TRUE),
			temp_sd = sd(data3$max.temp, na.rm = TRUE)
			# avg_asql = mean(asq_light, na.rm = TRUE),
			# asql_sd = sd(asq_light, na.rm = TRUE),
			# avg_asqh = mean(asq_heavy, na.rm = TRUE),
			# asqh_sd = sd(asq_heavy, na.rm = TRUE)
