...
data2 <- data2 %>%
	...
	rowwise() %>% 
	mutate(asq_light = mean(c(asq_1a, asq_2a, asq_3a, asq_4a, asq_5a, asq_6a, asq_7a, asq_8a, asq_9a), na.rm = TRUE)) %>%
	ungroup() # %>% 
...
data2 <- data2 %>% 
    mutate_at(vars(starts_with("asq_")), as.numeric) %>%
    rowwise() %>% 
    mutate(asq_heavy = mean(c(asq_10a, asq_11a, asq_12a, asq_13a, asq_14a,asq_15a), na.rm = TRUE)) %>%
    ungroup() #%>% 
...
data2 %>% 
	group_by(Groups) %>%
	summarise(
		avgTemp = mean(max.temp, na.rm = TRUE)
	)
...
```{R ---> Bar chart descriptives}
#	EXPLORING THE DATA OF THE 3 GROUPS
ggplot(data = data2) +
	geom_bar(mapping = aes(x = Groups, fill = Groups))

ggplot(data = data2, aes(fill = Groups)) +
	geom_bar(mapping = aes(x = Groups))
```

```{r ---> Point graphs max.temp}
#		GROUP PLOTS FOR max.temp FOR EACH GROUP = Version 1
ggplot(data2, aes(Groups, max.temp, colour = Groups)) +
  geom_point() +
  geom_point(data = ASQGpSummary, aes(y = avg_temp), colour = 'purple3', size = 3)

#		GROUP PLOTS FOR max.temp FOR EACH GROUP - WITH ERROR BARS = Version 2
ggplot() +
	geom_point(data = data2, aes(Groups, max.temp, colour = Groups)) +
	geom_point(data = ASQGpSummary, 
...
#		GROUP PLOTS FOR max.temp FOR EACH GROUP - WITH ERROR BARS = Version 3
...
```{r ---> Scatterplot for ASQ}
#		GROUP PLOTS FOR asq_light by asq_heavy FOR EACH GROUP = Version 1
(d <- ggplot(data2, aes(asq_light, asq_heavy), na.rm = TRUE) +
		geom_point(aes(colour = Groups))
) + scale_colour_hue(h = c(180, 360), l = 50, c = 160)

#		GROUP PLOTS FOR asq_light by asq_heavy FOR EACH GROUP = Version 2
