...
data0 <- read_csv("ASQ_pain_pilot_2019_04_19_v2.csv",
				  col_names = TRUE,
				  col_types = NULL,
				  quoted_na = FALSE)
...
data97 <- read.csv("ASQ_pain_pilot_2019_04_19_v2.csv", header = TRUE)
#		RAW DATA ON GITHUB
# data2 <- as_tibble(data1)
#data3 <- read_csv(url('https://raw.githubusercontent.com/BrainStormCenter/ASQ_pilot/master/ASQ_pain_pilot_2019_04_18.csv'), header = TRUE)
...
# CHECK THAT THE DATA ARE OF THE CORRECT FORMAT/TYPE
```{r Factor check}
#	CHECK WHICH VARIABLES ARE CODED AS FACTORS AND THEIR LEVELS
dataFactors <- select_if(data21, is.factor)
...
data3 %>% 
	...
	summarise(
		avgTemp = mean(data3$max.temp, na.rm = TRUE)
	)
...
data3 %>%  #	THIS IS NOT WORKING
	ggplot(data = data3$asq_light, mapping = aes(x = data3$Groups)) +
	geom_boxplot()
...
vars_keep <- select(data4, asq_light, asq_heavy,Max.temp)
