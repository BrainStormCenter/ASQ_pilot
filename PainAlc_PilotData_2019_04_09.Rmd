...
ggplot(data = data3, mapping = aes(x = asq_light, color = Groups3)) +
...
# ggplot(data = data3, mapping = aes(x = asq_heavy, color = Groups3)) +
# 	geom_freqpoly(binwidth = 0.1)
...
```{r Correlations 3 groups}
#	LEARNING HOW TO CALCULATE CORRELATIONS WITH A SMALLER DATASET
vars_keep <- select(data4, asq_light, asq_heavy,max.temp)

#	The results of this have NA in the off diagonal = Bad!
#cor(vars_keep[,c("asq_light", "asq_heavy", "max.temp")])
	
#	This works correctly by does not have any p-values = ok not great 
cor(vars_keep, use = "complete.obs", method = "pearson")

M <- cor(vars_keep, use = "complete.obs")
rcorr(as.matrix(vars_keep))
