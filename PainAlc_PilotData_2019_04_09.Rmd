...
dat.reasq.pre  <- select(dat.asq3, ID, sex, 
					 asq_light, asq_heavy,
					 ends_with("a"),
					 ends_with("z"))

dat.reasq.post  <- select(dat.asq3, ID, sex, 
						  asq_light, asq_heavy,
						  ends_with("a"))


#		Z-SCORES BY SEX
dat.reasq.post <- dat.reasq.post %>% 
	group_by(sex) %>%
	mutate_at(vars(ends_with("a")),
			  list(z = ~as.vector(scale(.)))) %>% 
	ungroup()
...
z.diff <- dat.reasq.post - dat.reasq.pre

z.diff2  <- select(z.diff, ID, sex, 
				   asq_light, asq_heavy,
				   ends_with("z"))

write.csv(dat.reasq.pre, "dat.reasq.pre.csv", row.names = FALSE)
write.csv(dat.reasq.post, "dat.reasq.post.csv", row.names = FALSE)
...
dat.asq2 %>% 
	group_by(sex) %>%
	summarise(
		avgTempSex = mean(max.temp, na.rm = TRUE)
	)
...
# ASQ_Zsmry <-
	...
	group_by(sex) %>%
	summarise(
		avgASQ.lzSex = mean(asq_lz, na.rm = TRUE),
		avgASQ.hzSex = mean(asq_hz, na.rm = TRUE)
	)
View(ASQ_Zsmry)

ggplot(data = ASQ_Zsmry) +
	geom_bar(mapping = aes(x = avgASQ.lzSex, fill = sex))
...
ggplot(dat.asq, aes(Groups, max.temp, colour = Groups), na.rm = TRUE) +
  geom_point() +
  geom_point(data = ASQ_Smry, aes(y = avg_temp), 
  		   colour = 'purple3', size = 3)

#		GROUP PLOTS FOR max.temp FOR EACH GROUP - WITH ERROR BARS = Ver 2
ggplot() +
	geom_point(data = dat.asq, aes(Groups, max.temp, colour = Groups)) +
	geom_point(data = ASQ_Smry, 
			   aes(Groups, avg_temp), colour = 'purple3', size = 5) +
	geom_errorbar(
		data = ASQ_Smry,
		aes(Groups, avg_temp, ymin = avg_temp - temp_sd,
			ymax = avg_temp + temp_sd), colour = 'green',
		width = 0.5) 

#		GROUP PLOTS FOR max.temp FOR EACH GROUP - WITH ERROR BARS = Ver 3
q <- ggplot(ASQ_Smry, aes(Groups, avg_temp,colour = Groups))

z <- q + geom_linerange(aes(ymin = ASQ_Smry$avg_temp - ASQ_Smry$temp_sd,
							ymax = ASQ_Smry$avg_temp + ASQ_Smry$temp_sd))
z <- z + geom_pointrange(aes(ymin = ASQ_Smry$avg_temp - ASQ_Smry$temp_sd,
							 ymax = ASQ_Smry$avg_temp + ASQ_Smry$temp_sd))
z <- z + geom_errorbar(aes(ymin = ASQ_Smry$avg_temp - ASQ_Smry$temp_sd,
						   ymax = ASQ_Smry$avg_temp + ASQ_Smry$temp_sd),
					   width = 0.2)
print(z)
...
(d <- ggplot(dat.asq, aes(asq_light, asq_heavy), na.rm = TRUE) +
		geom_point(aes(colour = Groups))
) + scale_colour_hue(h = c(180, 360), l = 50, c = 160)

(dz <- ggplot(dat.asq2, aes(asq_lz, asq_hz), na.rm = TRUE) +
		geom_point(aes(colour = Groups))
) + scale_colour_hue(h = c(180, 360), l = 50, c = 160)
