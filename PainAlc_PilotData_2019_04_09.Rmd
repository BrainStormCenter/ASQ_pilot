...
data2[data2$asq_15a  == "#DIV/0!", "asq_15a"] <- "NA"

#	CREATING TEMPORARY DATA SUBSET
tmp <- select(data2, participant_id, max.temp, asq_15a)
...
tmp <- select(data2, participant_id, contains("asq"))

tmp2 <- as_tibble(select(data2, participant_id, contains("asq_15")))

tmp3 <- as_tibble(select(data2, contains("asq_15")))
tmp3
tmp4 <- mutate_if(tmp3, is.factor, as.numeric(as.character(tmp3,"asq_15a")))

tmp5

tmp[tmp$starts_with("asq") == "#DIV/0!", starts_with("asq")] <- "NA"


filter(tmp, asq_15a == "#DIV/0!") <- "NA" 
...
#	COMPUTE MEAN ASQ VALUES
mutate(asq_sm, asq_l = mean(asq_14a + asq_15a)	  
	   )
