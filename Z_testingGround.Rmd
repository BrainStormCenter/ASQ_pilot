...
ggplot(data2, aes(max.temp, asq_heavy, shape=Groups, 
...
ggplot(data2, aes(max.temp, asq_light, shape=Groups, 
				  colour=Groups, fill=Groups)) + 
	# geom_smooth(method = "gam", na.rm = TRUE) +
	geom_smooth(method = "auto", na.rm = TRUE) +
	# geom_smooth(se = FALSE, na.rm = TRUE) +
...
sp <- ggplot(data2, aes(x = max.temp, y = asq_light)) +
	geom_point(shape = 1) + 
	facet_grid(. ~ Groups)
print(sp)

#		MULTI-PANE OF ASQ-LIGHT AND MAX.TEMP BY GROUP
ggplot(data2, aes(x = max.temp, y = asq_light,
				  shape = Groups, 
				  colour = Groups,
				  fill = Groups)) +
	geom_point(shape = 1) + 
...
ggplot(data2, aes(x = max.temp, y = asq_heavy,
				  shape = Groups, 
				  colour = Groups,
				  fill = Groups)) +
	geom_point(shape = 1) + 
